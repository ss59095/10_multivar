...
```{r}
weather <- read.csv("../Data/fieldweatherdata.csv")
...
lm_dayl.apr <- lm(strength_gtex ~ mean_dayl.s.Apr,
                  data = weather) %>%
  tidy() %>%
...
lm_prp.oct <- lm(strength_gtex ~ sum_prcp.mm.Oct,
   data = weather) %>%
  tidy() %>%
  filter(term!="(Intercept)") %>%
...
lm_dayl.prcp <- lm(strength_gtex ~ mean_dayl.s.Apr + 
                   sum_prcp.mm.Oct,
   data = weather) %>%
  tidy() %>%
  filter(term != "(Intercept)") %>%
...
lm_daylapr.daylmay <- lm(strength_gtex ~ mean_dayl.s.Apr + 
                  mean_dayl.s.May,
   data = weather) %>%
  tidy() %>%
  filter(term!="(Intercept)") %>%
...
```{r vif uncorrelated}
# Uncorrelated
lm(strength_gtex ~ mean_dayl.s.Apr +
     mean_dayl.s.Oct,
   data = weather) %>%
  vif()
```
